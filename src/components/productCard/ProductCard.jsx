...
function ProductCard() {
    ...
    const addCart=(product)=>{
        const { time, ...productData } = product; // Extract time separately
        const unixTimestamp = time.toMillis(); // Convert _Timestamp to UNIX timestamp

        // Dispatch action with serializable payload
        dispatch(addToCart({ ...productData, time: unixTimestamp }));
                                    ...
                                    <div className="h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out    border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                        ...
                                        <div className="p-5 border-t-2">
                                            ...
                                            <div className=" flex justify-center">
                                                <button onClick={() => addCart(item)} type="button" className="focus:outline-none text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full  py-2">Add To Cart</button>
